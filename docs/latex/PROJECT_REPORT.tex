...
\midrule
Use case & \path{auth/auth-use-cases.md}, \path{groups/group-use-case-diagram.md}, \path{friends/friend-use-case-diagram.md}, \path{transactions/transaction-use-cases.md}, \path{settlements/settlement-use-cases.md} \\
Sequence & \path{auth/auth-sequence-diagrams.md}, \path{groups/group-sequence-diagrams.md}, \path{friends/friend-sequence-diagrams.md}, \path{transactions/transaction-sequence-diagram.md}, \path{settlements/settlement-sequence-diagram.md} \\
Activity & \path{design/activity-diagrams.md} \\
State machine & \path{design/state-machine-diagrams.md} \\
Class & \path{class\_diagram.md} \\
Extra sketches & \path{auth/auth-architecture.md}, \path{groups/group-architecture.md}, \path{friends/friend-architecture.md}, \path{architecture/transaction-settlement-architecture.md} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection{How to maintain the system}
\begin{enumerate}
	\item Repair bugs and faults discovered after deployment. Issues should be reproduced, fixed on a feature branch, reviewed through pull requests, and merged following the project's CI checks.
	\item Modify or add functionality to improve performance, reliability, or usability. Feature work follows the same branching and review process; backward-compatible changes should be prioritised when the service is live.
\end{enumerate}

\subsubsection{Future improvements}
\begin{enumerate}
  \item Consider general content-protection and access-control measures for any sensitive shared media; examples include stricter authentication/authorization, short-lived signed URLs, watermarking, and rate-limiting.
  \item Implement real-time push notifications (e.g. WebSockets or server-sent events) to notify users of invites, settlements, or friend activity.
\end{enumerate}

\subsubsection{Scalability and updates}
\begin{enumerate}
	\item Scalability can be managed using load distribution and container orchestration (Docker + load balancers, or Kubernetes) to scale the API and frontend instances as traffic grows.
	\item System updates are performed through continuous integration and controlled rollouts. Maintain semantic versioning, automated tests, and migration scripts; strive to preserve backward compatibility for client integrations.
\end{enumerate}
